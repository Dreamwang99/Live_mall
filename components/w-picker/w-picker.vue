	...
	<view class="w-picker">
		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			...
			<view class="w-picker-view" v-if="mode=='selector'">
				<picker-view :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in data" :key="index">{{item.title}}</view>
					</picker-view-column>
				</picker-view>
			</view>
				...
				switch(mode){
					...
					case "region":
						...
						if(!_this.hideArea){
							...
							_this.resultStr=province.label+city.label+area.label;
							_this.provincial=province.label;
							_this.market=city.label;
							_this.district=area.label;
